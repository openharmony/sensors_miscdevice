...
int32_t MiscdeviceServiceProxy::VibrateFd(int32_t vibratorId, int32_t fd, int32_t usage)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option;
    ...
    if (!data.WriteInt32(fd)) {
        MISC_HILOGE("WriteUint32 fd failed");
        return WRITE_MSG_ERR;
    }
    if (!data.WriteInt32(usage)) {
        MISC_HILOGE("WriteUint32 usage failed");
    ...
    sptr<IRemoteObject> remote = Remote();
    CHKPR(remote, ERROR);
    int32_t ret = remote->SendRequest(VIBRATE_FD, data, reply, option);
    if (ret != NO_ERROR) {
        HiSysEvent::Write(HiSysEvent::Domain::MISCDEVICE, "MISC_SERVICE_IPC_EXCEPTION",
            HiSysEvent::EventType::FAULT, "PKG_NAME", "VibrateFd", "ERROR_CODE", ret);
        MISC_HILOGE("sendRequest ret : %{public}d", ret);
    }
    return ret;
}
