...
#include "ishdhapticsupported_fuzzer.h"
...
bool IsHdHapticSupportedFuzzTest(const uint8_t *data, size_t size)
{
    if (data == nullptr || size < U32_AT_SIZE) {
        return false;
    }
    OHOS::Sensors::IsHdHapticSupported();
    ...
    OHOS::Sensors::IsHdHapticSupportedEnhanced(identifier);
    return true;
}
} // namespace OHOS
...
{
    OHOS::IsHdHapticSupportedFuzzTest(data, size);
    return 0;
}