...
#include "playvibratoreffectstub_fuzzer.h"
...
void SetUpTestCase()
{
    ...
    TokenInfoParams infoInstance = {
        ...
        .perms = perms,
        .acls = nullptr,
        .processName = "PlayVibratorEffectStubFuzzTest",
        .aplStr = "system_core",
    };
...
bool OnRemoteRequestFuzzTest(const uint8_t *data, size_t size)
{
    ...
    std::string effect = "haptic.effect.soft";
    datas.WriteString16(Str8ToStr16(effect));
    int32_t loopCount = 0;
    startPos += GetObject<int32_t>(data + startPos, size - startPos, loopCount);
    datas.WriteInt32(loopCount);
    int32_t usage = 0;
    ...
    g_service->OnRemoteRequest(static_cast<uint32_t>(IMiscdeviceServiceIpcCode::COMMAND_PLAY_VIBRATOR_EFFECT),
        datas, reply, option);
    return true;
}
} // namespace Sensors
