...
bool OnRemoteRequestFuzzTest(const uint8_t *data, size_t size)
{
    ...
    size_t startPos = 0;
    int32_t lightId = 0;
    startPos += GetObject<int32_t>(data + startPos, size - startPos, lightId);
    datas.WriteInt32(lightId);
    int32_t singleColor = 0;
    startPos += GetObject<int32_t>(data + startPos, size - startPos, singleColor);
    datas.WriteInt32(singleColor);
    LightAnimationIPC animationIPC;
    datas.WriteParcelable(&animationIPC);
    datas.RewindRead(0);
    MessageParcel reply;
    MessageOption option;
    g_service->OnStartFuzz();
    g_service->OnRemoteRequest(static_cast<uint32_t>(IMiscdeviceServiceIpcCode::COMMAND_TURN_ON),
        datas, reply, option);
    return true;
}
} // namespace Sensors
