...
#include "stopvibratebysessionidstub_fuzzer.h"
...
void SetUpTestCase()
{
    ...
    TokenInfoParams infoInstance = {
        ...
        .perms = perms,
        .acls = nullptr,
        .processName = "StopVibrateBySessionIdStubFuzzTest",
        .aplStr = "system_core",
    };
...
bool OnRemoteRequestFuzzTest(const uint8_t *data, size_t size)
{
    ...
    GetObject<uint32_t>(data + startPos, size - startPos, sessionId);
    datas.WriteParcelable(&identifier);
    datas.WriteUint32(sessionId);
    MessageParcel reply;
    MessageOption option;
    g_service->OnStartFuzz();
    g_service->OnRemoteRequest(static_cast<uint32_t>(IMiscdeviceServiceIpcCode::COMMAND_STOP_VIBRATE_BY_SESSION_ID),
        datas, reply, option);
    return true;
}
} // namespace Sensors
