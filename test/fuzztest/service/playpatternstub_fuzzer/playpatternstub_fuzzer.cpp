...
#include "playpatternstub_fuzzer.h"
...
bool OnRemoteRequestFuzzTest(const uint8_t *data, size_t size)
{
    ...
    size_t startPos = 0;
    startPos += GetObject<int32_t>(data + startPos, size - startPos, identifier.position);
    datas.WriteParcelable(&identifier);
    VibratePattern pattern;
    datas.WriteParcelable(&pattern);
    ...
    g_service->OnRemoteRequest(static_cast<uint32_t>(IMiscdeviceServiceIpcCode::COMMAND_PLAY_PATTERN),
        datas, reply, option);
    return true;
}
} // namespace Sensors
